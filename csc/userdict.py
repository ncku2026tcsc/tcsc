...
_CAP = 8            # 多音字讀音組合上限
_MAX_LEVEL = 6     # 最高段（含單字，使用者要求一律 6 段）
...
def _candidates_for(gold: str, i: int):
    """差異位置 i 的候選詞：所有『含 i 的 2/3/4 字窗』+ 單字；短的優先、去重保序。
    供使用者批准 3、4 字的名詞/詞組（不只 1~2 字）。"""
    out, seen = [], set()
    for L in (2, 3, 4):                                # 2、3、4 字窗
        lo = max(0, i - L + 1)
        hi = min(i, len(gold) - L)                     # 起點上限：窗不超出字串、且含 i
        for start in range(lo, hi + 1):
            w = gold[start:start + L]
            if w and w not in seen:
                seen.add(w)
                out.append(w)
    if gold[i] not in seen:                            # 最後給單字選項
        out.append(gold[i])
    return out
